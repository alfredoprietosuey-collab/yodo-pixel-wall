...
function renderWorld(canvas, pct, seedStr){
  ...
  const cssH = canvas.clientHeight || Math.round(cssW * 0.5);
  ...
  // 1) Dry map: same pixel art, just washed-out (still colorful)
  ctx.save();
  ctx.imageSmoothingEnabled = false;
  ctx.filter = "saturate(0.35) contrast(1.05) brightness(1.10)";
  ctx.globalAlpha = 0.85;
  ctx.drawImage(base, 0, 0, cssW, cssH);
  ctx.restore();

  // 2) How many land pixels to wet
  const p = clamp(pct, 0, 100);
  const total = landMeta.idx.length;
  let n = Math.floor((p/100) * total);
  if (p > 0 && n < 120) n = Math.min(total, 120);
  ...
  // 3) Full-color reveal layer from base
  ...
  for (let y=0; y<h; y++){
    for (let x=0; x<w; x++){
      ...
      if (!chosen.has(idx1)){
        ...
        // Slight boost for “wet”
        d[i]   = Math.min(255, d[i]   * 1.02);
  ...
  ctx.save();
  const grd = ctx.createRadialGradient(cssW*0.5, cssH*0.55, cssH*0.15, cssW*0.5, cssH*0.55, cssW*0.85);
